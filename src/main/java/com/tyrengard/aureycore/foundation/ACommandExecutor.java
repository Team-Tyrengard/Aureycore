...
import com.tyrengard.aureycore.common.struct.Trie;
...
import com.tyrengard.aureycore.common.stringformat.SpecialChars;
...
public abstract class ACommandExecutor implements TabExecutor {
	...
	protected final void createHelpCommands(String header, ChatColor baseColor, ChatColor accentColor) {
		for (Entry<String, Trie<CommandDeclaration<Player>>> entry : playerCommands.entrySet()) {
			...
			if (baseCommand != null) 
				mainHelpOutput.add("" + accentColor + SpecialChars.BULLET + mainCommand + ChatColor.WHITE + " - " 
					+ baseCommand.getShortDescription());
			mainHelpOutput.add("" + accentColor + SpecialChars.BULLET + " /" + mainCommand + " help, ?" + ChatColor.GRAY + 
				" - Shows this help menu");
			mainHelpOutput.add("" + accentColor + SpecialChars.BULLET + " /" + mainCommand + " [subcommand] help, ?" + ChatColor.GRAY + 
					" - Shows command info");
			
			for (CommandDeclaration<Player> cd : cdTrie.getValues()) {
				if (cd.getShortDescription() != null) {
					mainHelpOutput.add("" + accentColor + SpecialChars.BULLET + " /" +  cd.getFullCommand() + ChatColor.GRAY + " - " + 
